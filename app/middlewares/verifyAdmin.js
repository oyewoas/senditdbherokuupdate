...
const verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    const selectUser = 'SELECT * FROM users WHERE user_id = $1 AND isadmin = $2';
    const { rows } = await dbQuery.query(selectUser, [decoded.userId, decoded.admin]);
    const dbResponse = rows[0];
    if (!dbResponse) {
      badRequest.description = 'The token you provided is invalid';
      return res.status(400).send(badRequest);
    }
    req.user = { user_id: decoded.userId, isadmin: decoded.admin };
    next();
  } catch (error) {
